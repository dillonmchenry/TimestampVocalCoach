...
This sprint is non-implementation. Its deliverables are design documents that feed directly into Sprint 2 implementation work.
...
flowchart LR
    ...
    subgraph note [NoteMeasurementV2]
        ...
        NEW["vibrato_rate_hz, vibrato_extent_cents, scoop_cents, sustain_jitter, onset_ramp_db_per_s, envelope_shape..."]
    end
...
- **New function `_measure_pitch_shape()`** in [align_v2.py](vocal_coach/align_v2.py) — receives the same `(note, pitch, core_start, core_end)` args as `_measure_pitch()`. Computes:
  - `vibrato_rate_hz: Optional[float]` — dominant oscillation frequency of the cents curve (autocorrelation peak)
  - `vibrato_extent_cents: Optional[float]` — amplitude of that oscillation
  - `scoop_cents: Optional[float]` — signed pitch offset at attack onset relative to the core median (negative = scoop from below)
  - `sustain_jitter_cents: Optional[float]` — std-dev of cents in the core window
  - `release_slope_cents_per_s: Optional[float]` — pitch trajectory in the final ~100ms
...
- **New function `_measure_loudness_shape()`** — receives `(note, loudness, start_s, end_s)`. Computes:
  ...
  - `sustain_rms_std: Optional[float]` — std-dev of RMS in the core window
...
Cross-dimensional moments don't fit cleanly into the existing four categories (`pitch`, `technique`, `alignment`, `dynamics`). Two options:

- **Option A: primary dimension** — classify each composite moment under the dimension that the coaching copy primarily addresses. `breath_support` -> `pitch` (because the user perceives it as a pitch problem). `controlled_crescendo` -> `dynamics`.

- **Option B: new category** — add a `"technique_quality"` or `"vocal_health"` category for moments that span dimensions. This aligns better with Sprint 2's pedagogical direction but requires UI changes to the filter row.
...
## Open Questions for Sprint 2 Scoping

- Which sub-note features to prioritise: vibrato characterisation is likely highest-value because the signal is strong and the pedagogical mapping is clear. Scoop detection is second.
- Whether to add a new highlight category or map composite moments onto existing categories.
- Minimum note duration for sub-note features to be reliable (vibrato needs ~300ms+; scoop needs ~150ms of attack window).
- Whether to compute sub-note features for the reference track as well (enables "reference uses vibrato at 5.2 Hz here, you used 3.8 Hz" — richer comparative feedback).