...
def validate_itv(ph_itv):
    # ph_itv [T, 2]
    res_ph_itv = []
    for idx in range(ph_itv.shape[0]):
        itv = ph_itv[idx]
        if itv[0] >= itv[1]:
            raise RuntimeError("The ph duration should be positive")
        res_ph_itv.append([itv[0], itv[1]])
    res_ph_itv = np.array(res_ph_itv)
    return res_ph_itv
...
def midi_pitch_eval(mid_gt, mid_pred, offset_ratio=0.2):
    ...
    # print('$$$$')
    # for idx in range(pitch_pred.shape[0]):
    #     pitch, itv = pitch_pred[idx], interval_pred[idx]
    #     print(pitch, itv[0], itv[1])
