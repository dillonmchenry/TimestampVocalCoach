...
class VlabelerEditsCount(Metric):
    ...
    def update(self, pred: tg.PointTier, target: tg.PointTier):
        ...
        assert pred[-1].mark == 0 and target[-1].mark == 0
...
class IntersectionOverUnion(Metric):
    ...
    def compute(self, phonemes=None, add_all=False):
        if phonemes is None:
            if add_all:
                all_sum = 0
                all_v = 0
                for k, v in self.intersection.items():
                    all_sum += (self.sum[k] - v)
                    all_v += v
                return round(all_v / all_sum, 6)
            else:
                return {
                    k: round(v / (self.sum[k] - v), 10) if (self.sum[k] - v) != 0 else 0
                    for k, v in self.intersection.items()
                }
...
class BoundaryEditDistance(Metric):
    ...
    def update(self, pred: tg.PointTier, target: tg.PointTier):
        ...
        for i in range(len(pred)):
            assert pred[i].mark == target[i].mark, f'{pred[i].mark}, {target[i].mark}'
...
class StyleAcc(Metric):
    ...
    def update(self, pred, target):
        ...
        if len(valid_indices) != 0:
            ...
            self.num += len(valid_indices)
            self.acc += correct

    def compute(self):
        if self.num == 0:
            return None
        return round(self.acc / self.num, 6)